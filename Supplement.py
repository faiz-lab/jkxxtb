...
def report(person):
    ...
    time_start = datetime(2021, 1, 25, 0, 1)
    
    while True:
        differential = time_start - time_finish
        
        if (differential.days * 86400 + differential.seconds) > 0:
            break
        
        time_start += timedelta(hours=12)
        update_time = time_start + timedelta(minutes=randint(1, 30))
        ...
        now = update_time.strftime("%Y-%m-%d %H:%M")
        ...
        update_data = {
            "params": {
                ...
                "lxdh": person["lxdh"], "nl": person["nl"], "tjsj": now, "xrywz": person["xrywz"],
                "sheng": person["sheng"], "shi": person["shi"], "qu": person["qu"], "jtdzinput": person["jtdzinput"],
                "gj": person["gj"], "jtgj": person["jtgj"], "jkzk": person["jkzk"], "jkqk": person["jkqk"],
                "tw": str(round(uniform(36.5, 36.8), 1)), "sd": period, "bz": person["bz"], "_ext": "{}"
            }
        }
        print_log(update_data["params"]["gh"] + "\t" + update_data["params"]["tw"] + "°C")
        url2 = "https://workflow.sues.edu.cn/default/work/shgcd/jkxxcj/com.sudytech.work.shgcd.jkxxcj.jkxxcj" \
               ".saveOrUpdate.biz.ext"
        ...
        if final_result.json()['result']["success"]:
            ...
            print_log("[" + final_result.json()['result']['errorcode'] + "]" + final_result.json()['result']['msg'])
